...
def test_returns_none_when_all_inputs_unset() -> None:
    assert _build_hardware_payload(None, None, None, None, None, None) is None


def test_only_cpu_flag_produces_cpu_field() -> None:
    assert _build_hardware_payload(4, None, None, None, None, None) == {"cpu": 4}
...
def test_only_hardware_json_loads_cleanly(tmp_path: Path) -> None:
    ...
    assert _build_hardware_payload(None, None, None, None, None, path) == {
        "cpu": 2,
        "memory": "8Gi",
        "gpu": 1,
        "gpu_memory": "16Gi",
...
def test_flag_wins_over_hardware_json_on_conflict(tmp_path: Path) -> None:
    ...
    result = _build_hardware_payload(16, None, None, None, None, path)
...
def test_missing_hardware_json_file_exits_nonzero(tmp_path: Path) -> None:
    ...
    with pytest.raises(typer.Exit) as excinfo:
        _build_hardware_payload(None, None, None, None, None, missing)
...
def test_invalid_json_exits_nonzero(tmp_path: Path) -> None:
    ...
    with pytest.raises(typer.Exit) as excinfo:
        _build_hardware_payload(None, None, None, None, None, path)
    assert excinfo.value.exit_code == 1
...
def test_gpu_model_flag_produces_gpu_model_field() -> None:
    assert _build_hardware_payload(None, None, None, None, "RTX 5080", None) == {
        "gpu_model": "RTX 5080"
    }


def test_gpu_model_accepted_in_hardware_json(tmp_path: Path) -> None:
    path = tmp_path / "hw.json"
    path.write_text(json.dumps({"gpu_model": "RTX 5090"}))
    assert _build_hardware_payload(None, None, None, None, None, path) == {
        "gpu_model": "RTX 5090"
    }
