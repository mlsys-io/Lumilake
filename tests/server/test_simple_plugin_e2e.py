...
async def test_sample_plugin_exercises_every_plugin_component(
    ...
    admin = await identity.resolve("demo-admin-token", logger)
    user = await identity.resolve("demo-user-token", logger)
    unknown = await identity.resolve("unknown-token", logger)
...
async def test_sample_plugin_submission_and_data_guards_block_requests(
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        sample_plugin.BLOCKED_PRINCIPALS.clear()
        sample_plugin.TOKENS["demo-nodata-token"] = PrincipalContext(
            principal_id="charlie",
            org_id="demo",
            external_id="charlie@example.com",
            principal_type="user",
        ...
        denied = await client.post(
            "/jobs",
            json=_submit_payload(),
            headers={"Authorization": "Bearer demo-nodata-token"},
        )
...
async def test_sample_plugin_list_endpoints_require_type_level_read(
    ...
    sample_plugin.TOKENS["demo-noscope-token"] = PrincipalContext(
        principal_id="dana",
        org_id="demo",
        external_id="dana@example.com",
        principal_type="user",
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        jobs = await client.get(
            "/jobs",
            headers={"Authorization": "Bearer demo-noscope-token"},
        )
