...
def test_generation_config_requires_model_without_api() -> None:
    """A locally-loaded backend has no model to fall back to. Pins the
    ``__post_init__`` validation in GenerationConfig (common.py) that rejects
    a missing/empty ``model`` when ``api`` is not set."""
...
def test_generation_config_resolved_model_defaults_when_api_mode_omits_model() -> None:
    ...
    ``config.api.model`` nor the top-level ``config.model`` is set. Pins
    GenerationConfig's ``model: str = ""`` default plus its
    ``resolved_model()`` method (common.py)."""
