...
def _batch(graphs, workflows=()):
    return SimpleNamespace(
        runtime_graphs={f"g{i}": g for i, g in enumerate(graphs)},
...
def test_batch_fanout_width_counts_rows_without_a_static_list(server_factory) -> None:
    server = server_factory()
    batch = _batch(
        [_graph([_op("n0", {"type": "value", "value": "x"})])],
        workflows=[SimpleNamespace(slice_length=4), SimpleNamespace(slice_length=2)],
    )
    assert server._batch_fanout_width(batch) == 6
