...
def test_row_aligned_fanout_shards_across_eligible_workers(
    ...
    # The optimizer assigns the fan-out node n1 to a single worker (gpu-0).
    # With two eligible GPU workers in the pool, the row-aligned fan-out must
    # be sharded across both, not left as a single node on one worker.
    rewritten = _rewrite(
        flowmesh_manager,
        _fanout_graph(),
        {
