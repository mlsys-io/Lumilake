...
def main(argv: list[str]) -> int:
    ...
    if env_file is None and not (args.database_url and args.s3_url):
        raise SystemExit(
            "No .env found and --database-url/--s3-url not provided. "
            "Pass --env-file or set DATABASE_URL and S3_URL in the environment."
        )
    ...
    info(f"[2/2] bundling s3://{cfg.bucket}/{s3_prefix}/news -> {s3_path}")
    with tempfile.TemporaryDirectory(prefix="lumilake-demo-news-") as stage:
        stage_path = Path(stage)
        count = download_news_tree(client, cfg.bucket, args.s3_prefix, stage_path)
        info(f"       downloaded {count} objects")
