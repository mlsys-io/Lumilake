...
def _compute_minio_listing() -> tuple[dict[str, int | None], list[str]]:
    ...
    Scans ``S3_URL``'s path (bucket + key prefix) when set, falling back to
    ``S3_USER_DATA_PREFIX``. ``file_sizes`` maps relative paths under that
    prefix to byte size; ``folder_paths`` is the set of folder prefixes implied.
    """
    endpoint = envs.S3_ENDPOINT
    access_key = envs.S3_ACCESS_KEY
    connection_value = envs.S3_CONNECTION_VALUE
    data_prefix_raw = envs.S3_URL_PREFIX or envs.S3_USER_DATA_PREFIX
