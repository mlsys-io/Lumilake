...
class FlowmeshRuntimeManager(BaseRuntimeManager):
    ...
    def _expand_row_aligned_fanout_candidates(
        ...
        *,
        ...
        HALO assigns each node to exactly one worker, so a fan-out node never
        accumulates more than one candidate and the shard rewrite never fires.
        A row-aligned fan-out node (partition total > 1, from its own static
        lists or its dependency chain) is inherently shardable across every
        eligible worker in the pool; expand its candidates so the rewrite
        splits it across those workers.
        """
    ...
    def _rewrite_worker_assignment_for_shards(
        *,
        ...
        # beyond the HALO-assigned worker). Ensure every candidate worker
        # receives its shard.
        for node_id, workers in node_candidates.items():
            if node_id not in shard_nodes_by_raw:
                continue
