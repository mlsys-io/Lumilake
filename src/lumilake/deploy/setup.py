...
from .docker_client import image_exists
...
def build_server_image(project_root: Path, image_tag: str) -> None:
    info(f"Building lumilake image (lumilake:{image_tag})...")
    cmd = ["docker", "build", "-t", f"lumilake:{image_tag}", "."]
...
def _start_server(
    ...
    *,
    ...
    if force_build or not image_exists(f"lumilake:{image_tag}"):
        build_server_image(project_root, image_tag)
    else:
        info(f"Image lumilake:{image_tag} already present, skipping build.")
    run(
