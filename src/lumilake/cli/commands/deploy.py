...
from lumilake.deploy import docker_client
from lumilake.deploy import flowmesh as fm_mod
from lumilake.deploy import setup as setup_mod
from lumilake.deploy import stop as stop_mod
from lumilake.deploy import update_flowmesh as update_fm_mod
from lumilake.deploy.doctor import DoctorFinding, run_doctor
from lumilake.deploy.env import (
    ENV_FILE_NAME,
    ENV_TEMPLATE_NAME,
    FLOWMESH_ENV_FILE_NAME,
    patch_env_value,
)
from lumilake.deploy.errors import DeployError
from lumilake.deploy.setup import SetupOptions, build_server_image
...
@app.command()
def build() -> None:
    """Build the lumilake server Docker image."""
    root = _find_project_root()
...
def restart(
    ...
    try:
        ...
        logging.error(str(exc))
        raise typer.Exit(code=1) from exc
    _run_setup(root, background=True, force_build=True)
...
def reset() -> None:
    ...
    try:
        ...
        logging.error(str(exc))
        raise typer.Exit(code=1) from exc
    _run_setup(root, background=True, reset=True, force_build=True)
