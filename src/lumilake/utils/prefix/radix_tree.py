...
from lumilake.utils.graph import reversed_topological_sort
...
def to_raw_prefix(prefix: StaticPrefixType) -> str | list[Message]:
    ...
    return "".join(prefix)  # type: ignore[arg-type]
...
class TemplatedRadixTree:
    ...
    def _add_to_node(
        ...
        while True:
            if self._is_key_empty(cur_key):
                # The insertion key matches the current internal node prefix exactly.
                # Attach a leaf here instead of descending with an empty remainder.
                leaf = cur_node.get_leaf()
                if leaf is None:
                    leaf = TemplatedNode(cur_node, None, label)
    ...
    def _remove_from_node(
        ...
        while True:
            if self._is_key_empty(cur_key):
                ...
                leaf.remove(node_id)
                node_stack.append((cur_node, None))
                last_node = leaf
                break
            assert not self._is_key_empty(cur_key)
