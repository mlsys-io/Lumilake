...
from lumilake.runtime.utils.pool import AsyncPool, MPAsyncPool
...
class EventLoop[E, R, Ctx](ABC):
    ...
    async def get_result(self, key: str) -> R:
        ...
        raise ValueError(
            f"Expect result_collector to be a pool. Got {type(self._result_collector)}"
        )
    ...
    async def pop_result(self) -> tuple[str, R]:
        ...
        raise ValueError(
            f"Expect result_collector to be a queue. Got {type(self._result_collector)}"
        )
...
class MPEventLoop(EventLoop[E, R, Ctx]):
    """
    Multiprocessing event loop that processes events in a separate process.
...
class MPConcurrentEventLoop(MPEventLoop[E, R, tuple[set[asyncio.Task], Ctx | None]]):
    """
    Multiprocessing concurrent event loop combining process isolation
    with concurrency.

    Extends MPEventLoop to support concurrent event processing within a
    separate process. Combines true parallel processing (multiprocessing)
    with concurrent I/O within that process.
    """
        ...
        **kwargs,
        ...
        while True:
            ...
            if isinstance(item, _EventBatch):
                assert item.keys is not None
                for key, event in zip(item.keys, item.events):
                    await self._handler_func((key, event), ctx)  # type: ignore[arg-type]
            else:
                await self._handler_func(item, ctx)
