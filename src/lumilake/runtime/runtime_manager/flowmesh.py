...
from lumilake.runtime.flowmesh_client import get_async_client
from lumilake.runtime.optimizer.base import Schedule
from lumilake.runtime.protocol import RequestCancelledError
from lumilake.runtime.request import RequestInfo
from lumilake.runtime.runtime_graph import Roles, RuntimeGraph, RuntimeGraphBuilder
...
class FlowmeshRuntimeManager(BaseRuntimeManager):
    ...
    def _validate_shard_intent_cardinality_contract(
        self,
        *,
        node_order: Sequence[str],
        node_map: Mapping[str, Mapping[str, Any]],
        shard_node_ids: set[str],
    ) -> None:
        if not shard_node_ids:
            return
        shard_set = {node_id for node_id in shard_node_ids if node_id in node_map}
        if not shard_set:
            return
        order_rank = {node_id: idx for idx, node_id in enumerate(node_order)}
        neighbors: dict[str, set[str]] = {node_id: set() for node_id in shard_set}
        for node_id in shard_set:
            deps = node_map.get(node_id, {}).get("dependsOn")
            if not isinstance(deps, list):
                continue
            for dep in deps:
                if not isinstance(dep, str) or dep not in shard_set:
                    continue
                neighbors[node_id].add(dep)
                neighbors[dep].add(node_id)

        visited: set[str] = set()
        for seed in sorted(
            shard_set, key=lambda node_id: order_rank.get(node_id, 10**9)
        ):
            if seed in visited:
                continue
            queue = [seed]
            ...
            while queue:
                ...
                for neighbor in neighbors.get(cur, ()):
                    if neighbor in visited:
                        continue
                    visited.add(neighbor)
                    queue.append(neighbor)
            component.sort(key=lambda node_id: order_rank.get(node_id, 10**9))

            expected_cardinality: int | None = None
            expected_from_node: str | None = None
            for node_id in component:
                ...
                if not lengths:
                    continue
                local_total = max(lengths)
                if any(length not in {1, local_total} for length in lengths):
                    raise ValueError(
                        f"Shard-intent precheck failed: node '{node_id}' has"
                        f" inconsistent static list lengths {sorted(set(lengths))}"
                    )
                if local_total <= 1:
                    continue
                if expected_cardinality is None:
                    expected_cardinality = local_total
                    expected_from_node = node_id
                    continue
                if local_total != expected_cardinality:
                    raise ValueError(
                        "Shard-intent precheck failed: static input cardinalities must"
                        " align within each shard-participating component. Expected"
                        f" {expected_cardinality} (from node '{expected_from_node}'),"
                        f" but node '{node_id}' has {local_total}."
                    )
    ...
    def _promote_shard_dependencies(
        self,
        *,
        node_order: Sequence[str],
        node_map: Mapping[str, Mapping[str, Any]],
        shard_intent_nodes: dict[str, list[str]],
        node_candidates: dict[str, list[str]],
        normalized_assignment: dict[str, list[str]],
    ) -> None:
        changed = True
        while changed:
            changed = False
            for node_id in node_order:
                shard_workers = shard_intent_nodes.get(node_id)
                ...
                for dep in deps:
                    if not isinstance(dep, str):
                        continue
                    existing = shard_intent_nodes.get(dep)
                    if existing is not None:
                        if set(existing) != set(shard_workers):
                            raise ValueError(
                                "Unsupported shard boundary for raw node "
                                f"'{node_id}': dependency '{dep}' requires "
                                "conflicting shard worker sets"
                            )
                        continue
                    ...
                    dep_lengths = (
                        self._collect_static_list_lengths(dep_data)
                        if isinstance(dep_data, Mapping)
                        else []
                    )
                    dep_has_static_partition = max(dep_lengths, default=0) > 1
                    dep_deps = dep_node.get("dependsOn")
                    dep_has_sharded_parent = isinstance(dep_deps, list) and any(
                        isinstance(parent, str) and parent in shard_intent_nodes
                        for parent in dep_deps
                    )
                    if not dep_has_static_partition and not dep_has_sharded_parent:
                        continue
                    promoted_workers = list(shard_workers)
                    shard_intent_nodes[dep] = promoted_workers
                    node_candidates[dep] = promoted_workers
                    for worker in promoted_workers:
                        assigned = normalized_assignment.setdefault(worker, [])
