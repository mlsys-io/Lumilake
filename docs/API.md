...
| Route | Purpose |
|-------|---------|
| `POST /api/v1/jobs/preview` | Compile workflow specs and generate an optimizer schedule without dispatching runtime work. Accepts an optional `optimizer` field (omitted falls back to `LUMILAKE_DEFAULT_OPTIMIZER`) and an optional `hardware` object (`{cpu, memory, gpu, gpu_memory, gpu_model}`) whose unset fields fall back to the server's `HARDWARE_*` env defaults. |
| `POST /api/v1/jobs` | Submit one or more workflows. Accepts an optional `optimizer` field (omitted falls back to `LUMILAKE_DEFAULT_OPTIMIZER`) and an optional `hardware` object (`{cpu, memory, gpu, gpu_memory, gpu_model}`) whose unset fields fall back to the server's `HARDWARE_*` env defaults. The role split matters: `cpu` / `memory` constraints filter every worker (CPU and GPU), but `gpu` / `gpu_memory` / `gpu_model` constraints only filter GPU-capable workers — so `hardware.gpu = 1` against a mixed CPU+GPU graph still admits CPU workers for the graph's CPU ops, instead of forcing the entire graph onto GPU workers. `gpu_model` matches a GPU worker when the string appears case-insensitively as a substring of any of its device names (e.g. `"RTX 5080"`). Submitting `hardware.gpu = 0` against a workflow that contains a GPU op (vLLM / transformers / diffusers / text-to-image) is rejected with HTTP 422. If no available worker satisfies the constraints the job stays queued and the scheduler keeps retrying each tick (`LUMILAKE_POLL_TIMEOUT_SECONDS` only bounds one acquisition attempt; on timeout the reservation is aborted and the items are returned to the queue). Cancel the job to release it. Use `GET /api/v1/optimizer` to enumerate valid optimizer names. |
