...
| Command | Purpose |
|---------|---------|
| `lumilake job submit <workflow>` | Submit a workflow spec. Pass `--optimizer <name>` to override the server default optimizer. Pass `--cpu` / `--memory` / `--gpu` / `--gpu-memory` / `--gpu-model` (or `--hardware-json`) to override the worker hardware for this job; unset fields fall back to the server's `HARDWARE_*` env defaults. `--cpu` / `--memory` filter every worker; `--gpu` / `--gpu-memory` / `--gpu-model` only filter GPU-capable workers, so `--gpu 1` is not a job-wide "GPU workers only" switch — a mixed CPU+GPU graph still admits CPU workers for its CPU ops instead of forcing the entire graph onto GPU workers. `--gpu-model` matches a GPU worker when the string appears case-insensitively as a substring of any of its device names (e.g. `--gpu-model "RTX 5080"`). A job with constraints no worker satisfies stays queued and the scheduler keeps retrying; cancel the job to release it. |
| `lumilake job preview <workflow>` | Compile and schedule a workflow without dispatching runtime work. Accepts the same `--optimizer` and `--cpu` / `--memory` / `--gpu` / `--gpu-memory` / `--gpu-model` / `--hardware-json` overrides as `submit`. |
...
| `lumilake job workflows <job_id>` | List FlowMesh workflows associated with the job. |
| `lumilake job logs show <job_id> <workflow_id> [--limit N] [--before C] [--after C] [--json]` | Fetch one page of paginated logs for a workflow. |
| `lumilake job logs stream <job_id> <workflow_id> [--cursor C]` | Stream live log entries for a workflow via SSE until interrupted. |
| `lumilake job logs download <job_id> <workflow_id> --output <dir>` | Download per-task archived logs as extracted `.jsonl` files. |
